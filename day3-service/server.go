...
//type Server struct{} //首先定义了结构体 Server，没有任何的成员字段。（day3-servcie进行了重定义）
//serviceMap sync.Map（内容为sync类型的服务映射表，用于寻找对应的服务）
...
//readRequest 方法中最重要的部分，即通过 newArgv() 和 newReplyv() 两个方法创建
//出两个入参实例，然后通过 cc.ReadBody() 将请求报文反序列化为第一个入参 argv，
...
//通过反射结构体已经映射为服务，但请求的处理过程还没有完成。从接收到请求到回复还差以
//下几个步骤：第一步，根据入参类型，将请求的 body 反序列化；第二步，调用 service.call，
//完成方法调用；第三步，将 reply 序列化为字节流，构造响应报文，返回。
...
//通过反射结构体已经映射为服务，但请求的处理过程还没有完成。
//从接收到请求到回复还差以下几个步骤：第一步，根据入参类型，
//将请求的 body 反序列化；第二步，调用 service.call，完成方法调用；
//第三步，将 reply 序列化为字节流，构造响应报文，返回。
// Server represents an RPC Server.Server表示一个RPC服务器
type Server struct {
	serviceMap sync.Map
}
...
//(找到对应的服务映射序列，就是找对应非服务，因为相应的映射服务存储在serviceMap)
