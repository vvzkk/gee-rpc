...
type Option struct {
	...
	ConnectTimeout time.Duration //0 means no limit,链接超时
...
//客户端连接超时，只net.Dial需要为 Dial 添加一层超时处理的外壳即可。
...
func (server *Server) handleRequest(cc codec.Codec, req *request, sending *sync.Mutex, wg *sync.WaitGroup, timeout time.Duration) {
	...
	go func() {
		...
		called <- struct{}{} //called 信道接收到消息，代表处理没有超时，继续执行 sendResponse。
		if err != nil {
			req.h.Error = err.Error()
			server.sendResponse(cc, req.h, invalidRequest, sending)
			sent <- struct{}{}
	...
	case <-time.After(timeout):
		...
		server.sendResponse(cc, req.h, invalidRequest, sending)
	case <-called: //time.After() 先于 called 接收到消息，说明处理已经超时，called 和
		// sent 都将被阻塞。在 case <-time.After(timeout) 处调用 sendResponse。
		<-sent
