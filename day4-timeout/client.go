...
//一个 Context 携带一个截止日期、一个取消信号和其他跨 API 边界的值。 Context 的方法可以被多个 goroutine 同时调用。
func (client *Client) Call(ctx context.Context, serviceMethod string, args, reply interface{}) error {
	...
	case <-ctx.Done():
		...
		return errors.New("rpc:call failed:" + ctx.Err().Error()) //f
	case call := <-call.Done:
		return call.Error
	}
}
...
func dialTimeout(f newClientFunc, network, address string, opts ...*Option) (client *Client, err error) {
	...
	//DialTimeout 与 Dial 类似，但需要超时。
	if err != nil {
		return nil, err // close the connection if client is nil
	}
	defer func() {
		if err != nil {
			_ = conn.Close()
	...
	select {
	case <-time.After(opt.ConnectTimeout): //如果连接创建超时，将返回错误。
